...
class CEM():
    def __init__(self, act_dim, horizon, reward_estimator, max_iters=5,
                 popsize=400, elites=0.1, epsilon=0.001, alpha=0.1, mix_coeff=0.05):
    ...
    def optimal_action(self, state, init_mean, init_var):
        """
        param: pi_actions: tensor of action trajectories
        """
        print('...planning...')
        ...
        while (t < self.max_iters) and (T.max(var) > self.epsilon):
            samples = int(self.popsize * (1 - self.mix_coeff))

            dist = TruncatedNormal(loc=mean, scale=var, a=-2, b=2) # range [-2,2] to avoid discontinuity at [-1,1]
            act_seqs = dist.sample(sample_shape=[samples,]) # output popsize x horizon x action_dims matrix
            ...
            exp_rewards, combined_actions = self.estimate_value(state, act_seqs) # returns pi_actions appended to CEM actions
